...
#'
#' Inside the \code{setup} block, the read-only reserved name \code{.data}
#' refers to the original object supplied to \code{data}. This allows setup code
#' such as \code{Y <- as.matrix(.data)} for a matrix or data frame, and
#' \code{S <- as.matrix(.data$sets)} for a named list.
#' The temporary \code{.data} binding is removed after setup has finished.
#'
...
#' }
#'
#' @param data A named list, data frame, matrix, or other object containing the
#'   observations used in the model. The original object is available as
#'   \code{.data} while the \code{setup} block is evaluated.
...
rtmb_model <- function(data, code, par_names = list(), init = NULL, view = NULL, fixed = NULL, silent = FALSE, gr_test = FALSE) {
  ...
  if (!is.null(input_names) && ".data" %in% input_names) {
    stop("'.data' is reserved for the original data object inside the setup block.", call. = FALSE)
  }
