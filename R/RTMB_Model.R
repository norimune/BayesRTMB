...
RTMB_Model <- R6::R6Class(
  ...
  public = list(
    ...
    calculate_satterthwaite_df = function(ad_obj, idx_fix_active = NULL, L_u_total = NULL) {
      ...
      # Step 2: Compute dH/dtheta_k via central differences
      eps <- 1e-4 * pmax(abs(par), 1)
      
      df_par <- rep(Inf, P)
      for (i in 1:P) {
        # Compute gradient of V_ii w.r.t. all parameters using dV = -V * dH * V
        grad_vi <- numeric(P)
        ...
        for (k in 1:P) {
          ...
          # dV_ii/dtheta_k = -(V %*% dH_k %*% V)[i,i] = -V[i,] %*% dH_k %*% V[,i]
          grad_vi[k] <- -as.numeric(V[i, ] %*% dH_k %*% V[, i])
        }
    ...
    optimize = function(laplace = TRUE, init = NULL, num_estimate = 1, control = list(),
      ...
      if (se_sampling) {
        ...
        } else if (auto_df || any(!is.infinite(df_t))) {
          if (auto_df) {
            est_dfs_all <- self$calculate_satterthwaite_df(ad_obj, idx_fix_active, L_u_total)
        ...
        if (!is.null(n_obs)) {
          ...
          est_dfs_all <- if (auto_df) self$calculate_satterthwaite_df(ad_obj, idx_fix_active, L_u_total)
                         else rep(df_t[1], L_u_total)
        } else {
