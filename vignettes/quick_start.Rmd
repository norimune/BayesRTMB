...
BayesRTMB can be installed from GitHub.
...
On Windows, Rtools is required to compile these dependencies.
...
If this returns `TRUE`, or reports that build tools are available, your setup is ready.
...
In this example, the estimated success probability `theta` is about 0.60.
...
- `plot_forest()` summarizes point estimates and credible intervals.
...
3. **[RTMB Internals and Inference Algorithms](rtmb_internals.html)**  
   Learn about internal processing such as MAP estimation, Laplace approximation, MCMC, and variational inference.

4. **Extended Quick Start**  
   The previous longer quick start is preserved as [Extended Quick Start](quick_start-long.html).
