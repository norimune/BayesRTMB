...
ラッパー関数が生成するコードも同じ規則に従います。データに相当する引数が複数ある場合は、内部で1つの名前付きlistにまとめられます。たとえば、生成コードに `.data$x` と `.data$y` があれば、`data = list(x = x, y = y)` として再現できます。一方、因子数などの構造設定は観測データに混ぜず、`nfactors <- 5` のような定数として生成コードの `setup` に表示されます。
