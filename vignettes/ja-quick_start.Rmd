...
`par_names` や `view` をモデル作成時に指定しておくと、要約結果の表示がわかりやすくなります。
`par_names` はパラメータの表示名を付けるために使い、`view` は `summary()` などで優先的に上に表示したい変数を指定するために使います。
...
`optimize(laplace = TRUE)` とすると、ランダム効果を **Laplace 近似**で積分消去しながら固定効果を推定できます。
