...
You can handle models including random effects (varying effects) using the `(1 | group)` notation, similar to the `lme4` package.

### Linear Mixed Models (rtmb_lmer)

For mixed models assuming a Gaussian (normal) distribution, `rtmb_lmer` is available and can be used with the same feeling as `lme4::lmer()`. This is a shortcut function that internally calls `rtmb_glmer(family = "gaussian")`.

Let's build a model taking into account that individual satisfaction varies depending on the group they belong to.
...
By specifying the `family` argument, you can estimate mixed models for binomial, ordered logistic, and other distributions.
