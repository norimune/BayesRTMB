...
After fitting a regression model (LM, GLM, GLMER), you can use `condal_effects()` and `simple_effects()` to analyze and visualize the results. These methods are currently available for models fitted using MCMC (`sample()`).

### Visualization with `condal_effects()`

The `condal_effects()` function is used to visualize the predicted values of a model. It is particularly powerful for understanding interaction effects.
...
While `condal_effects()` provides a visual overview, `simple_effects()` allows you to statistically examine the effect of a focal variable at specific levels of a moderator.
