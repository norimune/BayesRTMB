...
```{r eval=FALSE}
# 3つのクラスタを持つ混合正規分布モデル
mdl_mix <- rtmb_mixture(Y, k = 3)
mdl_mix$print_code()
mcmc_mix <- mdl_mix$optimize(num_estimate = 8)
mcmc_mix
```
...
回帰モデル（LM, GLM, GLMER）を適合させた後、`conditional_effects()` や `simple_effects()` を使用して結果の解析や可視化を行えます。これらのメソッドは現在、MCMC（`sample()`）で適合させたモデルで利用可能です。

### `conditional_effects()` による可視化

`conditional_effects()` 関数は、モデルの予測値を可視化するために使用されます。特に交互作用効果を理解するのに強力です。
...
`conditional_effects()` が視覚的な概観を提供するのに対し、`simple_effects()` では特定の調整変数のレベルにおける焦点変数の効果を統計的に検討できます。
