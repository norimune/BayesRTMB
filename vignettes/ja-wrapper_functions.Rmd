...
ガウシアン（正規分布）を仮定した混合モデルの場合、`lme4::lmer()` と同じ感覚で使える `rtmb_lmer` が利用可能です。これは内部的に `rtmb_glmer(family = "gaussian")` を呼び出しているショートカット関数です。
...
`family` 引数を指定することで、二項分布や順序ロジスティック分布などの混合モデルを推定できます。
...
すべてのラッパー関数で作成したモデルオブジェクトの推定メソッド（`sample()`, `variational()`）には、以下の共通オプションが用意されています。

### 並列計算 (parallel)

チェイン数が多い場合やモデルが重い場合は、`parallel = TRUE` を指定することで並列計算を行えます。
...
保存された結果は、後で `read_mcmc_csv()` 関数を使ってモデルオブジェクトに読み込むことができます。
