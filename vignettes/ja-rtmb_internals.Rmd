...
任意の `data` ブロックは、モデルで使うデータ名を宣言するためのブロックです。このブロックがある場合、`rtmb_model()` は、宣言された名前が入力された名前付きデータに含まれているかを確認します。

## setup

`setup` ブロックは、データ前処理のためのブロックです。

名前付きlistの要素やdata frameの列は、`setup` 内で名前を使って直接参照できます。加えて、読み取り専用の予約名 `.data` は、`rtmb_model(data = ...)` に渡した元のオブジェクトを参照します。行列、data frame、名前付きlistのいずれでも利用できます。`setup` で作成・更新された変数は内部のモデルデータに保存されますが、`.data` 自体は `setup` の終了時に削除されます。

ラッパー関数が作るコードでは、ユーザーが `print_code()` を見て同じモデルを再現できるように、必要な前処理を `setup` に明示する方針をとっています。データに相当する引数が複数ある場合は、内部で1つの名前付きlistにまとめ、`.data$x` や `.data$sets` のように参照します。因子数などのモデル構造に関する設定は、観測データに混ぜず、`setup` に定数として直接表示します。
