...
`model` ブロックでは、`lp <- 0` と書く必要はありません。BayesRTMB が内部で対数確率を自動的に足し合わせます。
...
混合分布モデルは、制約つきパラメータと複数初期値の重要性を学ぶよい例です。
...
混合分布は局所解が生じやすいため、複数の初期値から推定するのが有効です。

```{r}
init_mix <- list(
  theta = rep(1 / K, K),
  mu = quantile(Y, probs = seq(0.2, 0.8, length.out = K)),
  sigma = rep(sd(Y), K)
)

mdl_mix <- rtmb_model(data_mix, code_mix, init = init_mix)
fit_mix <- mdl_mix$optimize(num_estimate = 8)
```
...
推定中のパラメータを使った条件分岐は、AD計算で問題になることがあります。
