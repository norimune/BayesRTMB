...
Stan に近い感覚でモデルを書きつつ、R の中でC++へのコンパイルなしにそのままベイズ推定を進められます。

このページでは、BayesRTMB の位置づけと、パッケージを使う上での全体像をまとめます。
...
- **[RTMBの仕組みと推定のアルゴリズム](ja-rtmb_internals.html)**
...
このようなモデルでは、Laplace 近似を使ってランダム効果を周辺化（積分消去）できます。MAP 推定 (`optimize()`) ではデフォルトで `laplace = TRUE` となり、自動的に適用されます。
...
MCMC (`sample()`) でも指定は可能ですが、MCMCの場合は変量効果が問題なく推定できるため、基本的にはデフォルトの `laplace = FALSE` で問題ありません。
...
たとえばマルチレベルモデルであれば `rtmb_lmer()` を使います。
...
また、`rtmb_ttest()` を使うと、ベイジアン t 検定が簡単に実行できます。`bayes_factor()` メソッドで `fixed` 引数にリスト形式で「0 に固定したいパラメータと値」を指定するだけで、自動的に比較モデルの推定とベイズファクターの計算が行われます。
