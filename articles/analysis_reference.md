...
If you want the overall workflow first, see [Quick
Start](https://norimune.github.io/BayesRTMB/articles/quick_start.md). If
you want to write custom models, see [Writing Model
Codes](https://norimune.github.io/BayesRTMB/articles/writing_models.md).
If you want the details of the inference algorithms, see [RTMB Internals
and Inference
...
The Japanese version of this page is available at [BayesRTMB
...
- Check model comparison tools such as Bayes factors, WAIC, AIC, and
  BIC.
- Check how to fix parameters with `fixed` and how to build constrained
  models.
...
Most code chunks on this page use `eval = FALSE`. Some examples are
complete examples that can be run as-is, while others are partial code
fragments intended to be placed inside
[`rtmb_code()`](https://norimune.github.io/BayesRTMB/reference/rtmb_code.md).
When only `setup = { ... }`, `model = { ... }`, or `generate = { ... }`
is shown, read it as the code for that block.

The name `fit` is used as a placeholder for any fit object. In real
analyses, replace it with objects such as `fit_mcmc`, `fit_map`,
`fit_vb`, or `fit_classic`, depending on the estimation method.

The following terms are used throughout this page.

| Term | Meaning |
|----|----|
| fit object | Estimation result returned by [`sample()`](https://rdrr.io/r/base/sample.html), [`optimize()`](https://rdrr.io/r/stats/optimize.html), `variational()`, or `classic()` |
...
| Use classic AIC/BIC/ANOVA | [Section 7 Classic_Fit](#classic-fit) | `mdl$classic()`, [`AIC()`](https://rdrr.io/r/stats/AIC.html), [`BIC()`](https://rdrr.io/r/stats/AIC.html), [`anova()`](https://rdrr.io/r/stats/anova.html) |
| Compare models | [Section 8 Model comparison](#model-comparison) | `fit$bayes_factor()`, `fit$WAIC()`, [`AIC()`](https://rdrr.io/r/stats/AIC.html), [`BIC()`](https://rdrr.io/r/stats/AIC.html) |
| Fix parameters | [Section 9 fixed](#fixed-parameters) | `fixed = list(...)`, `mdl$fixed_model()` |
| Write a custom model | [Sections 10-14](#rtmb-code-blocks) | [`rtmb_code()`](https://norimune.github.io/BayesRTMB/reference/rtmb_code.md), [`Dim()`](https://norimune.github.io/BayesRTMB/reference/Dim.md), distributions, AD-taping notes |
| Use an old fit object with current methods | [Section 15.9](#upgrade-fit) | [`upgrade_fit()`](https://norimune.github.io/BayesRTMB/reference/upgrade_fit.md) |
...
For new users, it is often clearest to pass a data frame as `data` and
connect data-frame columns to model variables in `setup`.
...
[`summary()`](https://rdrr.io/r/base/summary.html), `estimate()`,
`diagnose()`, and `draws()` on the fit object.
...
| Goal | Recommendation |
|----|----|
| Final Bayesian inference | [`sample()`](https://rdrr.io/r/base/sample.html) |
...
| Fast point estimation | [`optimize()`](https://rdrr.io/r/stats/optimize.html) |
...
| Function | Purpose |
|----|----|
| [`rtmb_code()`](https://norimune.github.io/BayesRTMB/reference/rtmb_code.md) | Define a model with `setup`, `parameters`, `transform`, `model`, and `generate` blocks |
| [`rtmb_model()`](https://norimune.github.io/BayesRTMB/reference/rtmb_model.md) | Create an `RTMB_Model` from [`rtmb_code()`](https://norimune.github.io/BayesRTMB/reference/rtmb_code.md) and data |
| [`Dim()`](https://norimune.github.io/BayesRTMB/reference/Dim.md) | Declare parameter dimensions, constraints, types, and `random` status |
| `print_code()` | Inspect model code generated by a wrapper |
| [`upgrade_fit()`](https://norimune.github.io/BayesRTMB/reference/upgrade_fit.md) | Rebuild old saved fit objects with the current class definitions |

### 2.2 Wrapper Functions

| Function | Main use |
|----|----|
| [`rtmb_lm()`](https://norimune.github.io/BayesRTMB/reference/rtmb_lm.md) | Normal linear regression |
| [`rtmb_glm()`](https://norimune.github.io/BayesRTMB/reference/rtmb_glm.md) | Generalized linear models |
| [`rtmb_lmer()`](https://norimune.github.io/BayesRTMB/reference/rtmb_lmer.md) | Linear mixed models |
| [`rtmb_glmer()`](https://norimune.github.io/BayesRTMB/reference/rtmb_glmer.md) | Generalized linear mixed models |
| [`rtmb_ttest()`](https://norimune.github.io/BayesRTMB/reference/rtmb_ttest.md) | Bayesian/classic t tests, effect sizes, Bayes factors |
| [`rtmb_corr()`](https://norimune.github.io/BayesRTMB/reference/rtmb_corr.md) | Correlations, partial correlations, correlation matrices |
| [`rtmb_fa()`](https://norimune.github.io/BayesRTMB/reference/rtmb_fa.md) | Factor analysis |
| [`rtmb_irt()`](https://norimune.github.io/BayesRTMB/reference/rtmb_irt.md) | Item response theory models |
...
| [`rtmb_mediation()`](https://norimune.github.io/BayesRTMB/reference/rtmb_mediation.md) | Mediation analysis |
| [`rtmb_mixture()`](https://norimune.github.io/BayesRTMB/reference/rtmb_mixture.md) | Mixture distribution models |
| [`rtmb_table()`](https://norimune.github.io/BayesRTMB/reference/rtmb_table.md) | Contingency-table models |
| [`rtmb_loglinear()`](https://norimune.github.io/BayesRTMB/reference/rtmb_loglinear.md) | Log-linear models |
...
[`sample()`](https://rdrr.io/r/base/sample.html) is the standard route
for Bayesian inference.
[`optimize()`](https://rdrr.io/r/stats/optimize.html) and
`variational()` are useful for quick checks and initial-value search,
but MCMC is preferred for uncertainty evaluation and Bayes factors.

## 3. Common Fit-Object Methods

`MCMC_Fit`, `MAP_Fit`, `VB_Fit`, and `Classic_Fit` share several
methods. The meaning and availability of each method still depends on
the fit-object type.

In this section, `fit` means any fit object. Read it as `fit_mcmc` for
MCMC, `fit_map` for MAP, `fit_vb` for VB, and `fit_classic` for classic
estimation. In particular, `EAP()` and `MAP()` are mainly for MCMC/VB,
while classic point estimates are usually obtained with `estimate()`.
...
| `$print()` | Print [`summary()`](https://rdrr.io/r/base/summary.html) |
...
`pars = "parameters"` returns only the primary parameters declared in
the `parameters` block. This is usually what you want when reusing
estimates as initial values or fixed values.
...
`drop = TRUE` is the default. When only one parameter is selected, the
vector, matrix, or array itself is returned instead of a list. Use
`drop = FALSE` if you always want a list.
...
`EAP()` is a shortcut for `estimate(type = "EAP")`. `MAP()` returns
marginal MAP estimates by default.
...
For MCMC/VB, `pars = NULL` returns primary parameters by default. For
classic and MAP fits, results may also include estimated fixed effects,
transformed quantities, and generated quantities.

## 4. MCMC_Fit

`MCMC_Fit` is returned by
[`sample()`](https://rdrr.io/r/base/sample.html). It is the central
object for analyses that use the full posterior distribution, including
Bayes factors, WAIC, posterior predictive checks, rotations, and
...
`draws()` extracts posterior draws. By default, it returns fixed
parameters and available transformed/generated quantities.
...
For large models, narrow `pars` before extracting draws to avoid
unnecessary memory use.

### 4.3 `summary()` And `diagnose()`

[`summary()`](https://rdrr.io/r/base/summary.html) gives the estimation
table. `diagnose()` summarizes sampling quality.
...
Use these methods when you want derived quantities for each posterior
draw.
...
If a `generate` block contains `log_lik`, `generated_quantities()` is
the basis for WAIC and related pointwise predictive criteria.

### 4.5 Bridge Sampling And Bayes Factors

Bayes factors compare two fitted models. In BayesRTMB, Bayes factors are
usually computed from MCMC fits.
...
To compute WAIC, the model must provide pointwise log-likelihood values.
For wrappers, set `WAIC = TRUE` when creating the model.
...
`MAP_Fit` is returned by
[`optimize()`](https://rdrr.io/r/stats/optimize.html). It is useful for
fast point estimation, initial-value search, profiling, and
...
BayesRTMB keeps optimization history and selects the best run according
to the objective value and convergence status.
...
[`profile()`](https://rdrr.io/r/stats/profile.html) evaluates the
objective around selected parameters. It is useful for checking local
curvature, asymmetric uncertainty, and non-quadratic likelihood shape.
...
WAIC is usually interpreted as a posterior predictive criterion and is
most natural for MCMC. For MAP fits, WAIC is available when generated
`log_lik` values can be computed, but it should be interpreted as an
approximation.

## 6. VB_Fit

`VB_Fit` is returned by `variational()`. It approximates the posterior
distribution and is useful for exploring larger models or preparing
MCMC.
...
If the ELBO remains unstable, increase `iter`, try different initial
values, or use VB only as an exploratory tool before MCMC.

### 6.4 WAIC For VB Fits

WAIC can be computed from VB draws when `log_lik` is available.
Interpret it as an approximation, and prefer MCMC-based WAIC for final
reporting when feasible.

## 7. Classic_Fit

`Classic_Fit` is returned by `classic()`. It provides frequentist-style
output for supported wrapper models, especially flat-prior models.
...
| [`anova()`](https://rdrr.io/r/stats/anova.html) | Model comparison or analysis-of-variance tables |

### 7.2 ANOVA

Use [`anova()`](https://rdrr.io/r/stats/anova.html) for supported
classic fits.
...
These criteria are most natural for classic/MAP likelihood-based fits.
For Bayesian predictive comparison, use WAIC or posterior predictive
checks.

### 7.4 `robust_se()`

`robust_se()` returns a copy of the fit with robust standard errors by
default.
...
computes adjusted marginal means for supported models.
...
Use Bayes factors when the prior is part of the scientific question. Do
not compare models with incompatible data, likelihoods, or unintended
prior changes.
...
For VB, the ELBO is mainly a diagnostic and selection criterion among VB
runs.
...
`fixed` lets you fix one or more parameters to specified values. It can
be used when creating a model or by calling `$fixed_model()`.
...
sampling vector and supplied at fixed values during model evaluation.
...
Use `fixed` for fixing parameters to numeric values. Equality
constraints such as “make `mean0` and `mean1` equal” are usually better
expressed by changing the model parameterization so that both quantities
are built from the same underlying parameter.
...
[`rtmb_code()`](https://norimune.github.io/BayesRTMB/reference/rtmb_code.md)
separates model code into blocks.
...
Use `transform` for derived quantities that depend on parameters and may
need standard errors or ADREPORT-style treatment.
...
Use `generate` for quantities computed after estimation, such as
predictions, correlations, posterior predictive summaries, and pointwise
log-likelihoods.
...
Use `transform` when the quantity should participate in AD-based
reporting, standard errors, or model evaluation. Use `generate` when the
quantity is a post-estimation output. The printed code may look similar,
but internally the two blocks serve different purposes.
...
Use `random = TRUE` for latent variables or random effects to be
marginalized by Laplace approximation in optimization.
...
| Function | Purpose |
|----|----|
| [`log_sum_exp()`](https://norimune.github.io/BayesRTMB/reference/log_sum_exp.md) | Stable log-sum-exp, including AD values |
| [`softmax()`](https://norimune.github.io/BayesRTMB/reference/softmax.md) | Softmax probabilities |
| [`log_softmax()`](https://norimune.github.io/BayesRTMB/reference/log_softmax.md) | Log-softmax |
| [`inv_logit()`](https://norimune.github.io/BayesRTMB/reference/inv_logit.md) | Logistic inverse |
| [`logit()`](https://norimune.github.io/BayesRTMB/reference/logit.md) | Logit transform |
| [`log1p_exp()`](https://norimune.github.io/BayesRTMB/reference/log1p_exp.md) | Stable `log(1 + exp(x))` |
| [`rtmb_vector()`](https://norimune.github.io/BayesRTMB/reference/rtmb_vector.md) | Mutable AD-compatible vector |
| [`rtmb_array()`](https://norimune.github.io/BayesRTMB/reference/rtmb_array.md) | Mutable AD-compatible array |
| [`distance()`](https://norimune.github.io/BayesRTMB/reference/distance.md) | Euclidean distance |
| [`squared_distance()`](https://norimune.github.io/BayesRTMB/reference/squared_distance.md) | Squared Euclidean distance |

When writing categorical models with a baseline category, the natural
pattern can be used inside
[`rtmb_code()`](https://norimune.github.io/BayesRTMB/reference/rtmb_code.md).
...
RTMB records model evaluation on an automatic-differentiation tape. Code
that is simple, vectorized, and AD-compatible tapes faster and evaluates
more reliably.
...
Many `apply`-style functions are convenient but can be difficult to tape
inside AD code. Use explicit loops when necessary, especially for AD
values.

### 14.4 Notes On `if` And `ifelse`

Avoid branching on parameter-dependent quantities. Branching on data or
setup values is usually fine.
...
when you need a mutable container that will receive AD values inside a
loop.
...
Long loops, large generated quantities, and repeated matrix
decompositions can increase taping time. Keep generated quantities
focused on values that you will use.
...
- [`apply()`](https://rdrr.io/r/base/apply.html) over AD arrays.
...
when you want to reuse a saved fit object with the current class
definitions after updating the package.
...
If the embedded model object also needs current methods, upgrade it as
well.
...
- Use MCMC rather than VB for final uncertainty summaries when the model
  is difficult or highly nonlinear.
...
- For equality constraints, prefer reparameterizing the model instead of
  trying to express the constraint through `fixed`.
- Keep data-only calculations in `setup` and parameter-dependent
  calculations in `transform`, `model`, or `generate` as appropriate.
- Use
  [`rtmb_vector()`](https://norimune.github.io/BayesRTMB/reference/rtmb_vector.md)
  and
  [`rtmb_array()`](https://norimune.github.io/BayesRTMB/reference/rtmb_array.md)
  for mutable AD containers.
