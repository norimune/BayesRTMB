...
The `lp <- lp - log(sd)` term applies the Jeffreys scale prior
$`p(\sigma) \propto 1/\sigma`$ used by the JZS t-test.
...
the default prior for a freely estimated discrimination parameter is
`a ~ exponential(1 / 2)`, which has prior mean 2. Use
`prior_normal(a_rate = ...)` to change its rate.
