...
は、`lme4::glmer()` に近い式でモデルを書き、内部で BayesRTMB の
...
BayesRTMB
の推定法の優先度は、基本的に次の順で考えると分かりやすくなります。

1.  [`sample()`](https://rdrr.io/r/base/sample.html): MCMC
    で事後分布を直接調べる。
2.  [`optimize()`](https://rdrr.io/r/stats/optimize.html): MAP / ML
    を高速に確認する。random effects がある場合は Laplace 近似が便利。
3.  `variational()`: ADVI で近似事後分布をすばやく見る。
4.  `classic()`:
    [`prior_flat()`](https://norimune.github.io/BayesRTMB/reference/prior_flat.md)
    の wrapper モデルを頻度主義的に確認する。
...
出力は次のように、固定効果、分散パラメータ、事後区間、MCMC
診断が並びます。
...
`optimize(laplace = TRUE)` を使うと、ランダム効果を Laplace
...
MAP 推定では、点推定と Wald 型または sampling-based
の区間が表示されます。
...
MAP 推定は、モデルの挙動を素早く確認したい場合や、MCMC
...
[`rtmb_glmer()`](https://norimune.github.io/BayesRTMB/reference/rtmb_glmer.md)
は、`lme4` 風の random effect 記法を使います。
...
`gmc` は grand mean centering、`cwc` は centering within cluster
...
反復測定データがワイド型で保存されている場合でも、[`rtmb_glmer()`](https://norimune.github.io/BayesRTMB/reference/rtmb_glmer.md)
は内部でロング型に変換できます。Gaussian model で応答を `cbind(...)`
として書き、`within` に測定内要因を指定します。
...
この例では、`y_t1`, `y_t2`, `y_t3` の3列を内部でロング型に展開し、`time`
...
[`as.factor()`](https://rdrr.io/r/base/factor.html) されてから model
matrix
が作られます。カテゴリ変数を数値の連続量として誤って扱うことを避けたい場合に便利です。

### conditional_effects

推定後は、[`conditional_effects()`](https://norimune.github.io/BayesRTMB/reference/conditional_effects.md)
で予測値を可視化できます。
...
のように交互作用を指定すると、`x2` の代表値ごとに `x1`
の効果を描きます。

より詳しく交互作用を調べたい場合は、[`simple_effects()`](https://norimune.github.io/BayesRTMB/reference/simple_effects.md)
を使えます。
...
係数の不確実性、事後予測、Bayes factor、bridge sampling
を使う分析では、MCMC の結果を基準に考えるのが自然です。
...
random effects を含む mixed model では、`laplace = TRUE`
によってランダム効果を Laplace
...
VB
は便利ですが近似推論です。最終的な区間推定やモデル比較では、必要に応じて
MCMC と照合してください。

### classic

`classic()`
は、[`prior_flat()`](https://norimune.github.io/BayesRTMB/reference/prior_flat.md)
の wrapper モデルを頻度主義的に確認するための推定経路です。
...
informative prior を含むモデルでは `classic()`
は使いません。ベイズ推定として事前分布を入れる場合は、[`sample()`](https://rdrr.io/r/base/sample.html)、[`optimize()`](https://rdrr.io/r/stats/optimize.html)、`variational()`
を使います。

## 6. prior の設計

[`rtmb_glmer()`](https://norimune.github.io/BayesRTMB/reference/rtmb_glmer.md)
の prior は、`prior` 引数で指定します。

### prior_flat

[`prior_flat()`](https://norimune.github.io/BayesRTMB/reference/prior_flat.md)
は、追加の prior density を置かない指定です。wrapper のデフォルトです。
...
は、目的変数の範囲 `y_range` を使って、データの尺度に合った弱情報 prior
を作ります。
...
2値・カウント・順序カテゴリモデルでは、応答の尺度に合わせて、線形予測子上で極端になりすぎない弱情報
prior
が使われます。細かい値を手で決めるよりも、まず穏当な出発点を置きたい場合に便利です。
...
のようなモデル比較では、事前分布の設計が結果に直接影響します。[`prior_flat()`](https://norimune.github.io/BayesRTMB/reference/prior_flat.md)
は頻度主義的な確認には便利ですが、ベイズ的なモデル比較の prior
としては慎重に扱う必要があります。

そのような場合、[`prior_weak()`](https://norimune.github.io/BayesRTMB/reference/prior_weak.md)
...
は regularized
horseshoe、[`prior_ssp()`](https://norimune.github.io/BayesRTMB/reference/prior_ssp.md)
は spike-and-slab prior です。どちらも係数が多いモデルで使います。正則化
prior を使うモデルでは、MCMC で事後分布を確認することを勧めます。

### prior_jzs

[`prior_jzs()`](https://norimune.github.io/BayesRTMB/reference/prior_jzs.md)
は、JZS prior を使うための指定です。
...
で JZS prior を使った Bayes factor を計算できます。
...
`ordered` は通常の ordered logistic model です。`cutpoints`
...
3, 4 の場合、`1->2`, `2->3`, `3->4`
という遷移ラベルで係数が表示されます。これは、通常の `ordered` の
cutpoints とは意味が異なります。
...
`sigma_by` は、`gaussian`、`lognormal`、`student_t` のような連続 family
で使えます。
...
によって個人ごとに残差相関行列を作り、その個人内で `resid_time = "time"`
を使って AR(1) のラグを決めます。

この例では、`id` 内の反復測定の相関を `resid_corr = "ar1"`
で直接表現しているため、式には `(1 | id)` を入れていません。`(1 | id)`
と `resid_corr`
...
モデル比較には
`anova(fit1, fit2)`、[`AIC()`](https://rdrr.io/r/stats/AIC.html)、[`BIC()`](https://rdrr.io/r/stats/AIC.html)
も使えます。
...
[`sample()`](https://rdrr.io/r/base/sample.html) で推定します。

## 10. print_code() で内部モデルを見る

[`rtmb_glmer()`](https://norimune.github.io/BayesRTMB/reference/rtmb_glmer.md)
が作るモデルは、`print_code()` で確認できます。
...
現在の BayesRTMB では、生成される `model` ブロックはできるだけ sampling
syntax を使う方針です。
...
[`rtmb_glmer()`](https://norimune.github.io/BayesRTMB/reference/rtmb_glmer.md)
が作る `RTMB_Model` には、wrapper metadata も保存されます。特に
`extra$source`、`extra$prior_type`、`extra$marginal` は、`classic()` や
`optimize(marginal = "auto")` の動作に関わります。

## 11. 実践上の注意点

- 最終的な不確実性を見たい場合は、まず MCMC を考えます。
- `optimize(laplace = TRUE)` は、mixed model
  の高速な確認に向いています。
- `variational()` は便利な近似推論ですが、必要に応じて MCMC
  と照合してください。
- `classic()` は wrapper 由来かつ
  [`prior_flat()`](https://norimune.github.io/BayesRTMB/reference/prior_flat.md)
  のモデルを頻度主義的に確認するための経路です。
- bridge sampling や Bayes factor では prior
  の影響が大きいため、[`prior_weak()`](https://norimune.github.io/BayesRTMB/reference/prior_weak.md)
  や明示的な
  [`prior_normal()`](https://norimune.github.io/BayesRTMB/reference/prior_normal.md)
  など、目的に合った事前分布を設計してください。
- `family = "sequential"` は、通常の ordered logistic
  とは異なり、カテゴリ遷移ごとに異なる係数を持ちます。
- `sigma_by` と `resid_corr` は、連続量モデル、特に Gaussian
  系のモデルで使う機能です。

より低水準のモデルコードを書きたい場合は、[モデルコードの書き方](https://norimune.github.io/BayesRTMB/articles/ja-writing_models.md)
