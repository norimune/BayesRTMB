...
This page walks through the minimal workflow needed to start using
BayesRTMB.
...
BayesRTMB uses RTMB / TMB internally. For ordinary use, Windows users
can install the CRAN binary package without Rtools. Rtools is only
needed for source installation, development, or compiling custom TMB C++
templates.

If you install BayesRTMB from source, you can check whether Rtools is
available with the following code.
...
If this returns `TRUE`, or reports that build tools are available, your
source-install setup is ready. If Rtools is not found, install the
Rtools version that matches your R version, then restart R or RStudio.

## 1. Write a Minimal Model

We start with a small binomial model. Suppose that 6 successes were
observed out of 10 trials.
...
The `parameters` block declares the parameters to estimate. Here,
`theta` is defined as a parameter constrained between 0 and 1.

The `model` block specifies the sampling distribution and prior
distribution. `Y ~ binomial(Trial, theta)` says that the number of
successes `Y` follows a binomial distribution.
...
At this stage, estimation has not yet been run. `mdl` is an `RTMB_Model`
object that stores the model definition and data.

## 3. Run MAP Estimation

Use [`optimize()`](https://rdrr.io/r/stats/optimize.html) when you want
a fast point estimate. In BayesRTMB, this is treated as MAP estimation
when priors are present, and as something close to maximum likelihood
estimation when flat priors are used.
...
Use [`sample()`](https://rdrr.io/r/base/sample.html) when you want to
inspect the full posterior distribution. The settings below are
intentionally short for a quick start. For real analyses, use more
iterations.
...
The MCMC summary reports the mean, standard deviation, posterior
quantiles, ESS, and R-hat. As a rough diagnostic, R-hat should be close
to 1 and ESS should be sufficiently large.
...
MCMC results should be checked visually as well as numerically. Use
`draws()` to extract samples for a target parameter, then inspect
densities, traces, autocorrelation, and intervals.
...
Here we use the `debate` data and fit a multiple regression model in
which satisfaction `sat` is explained by `talk`, `perf`, and their
interaction.
...
when you want to view the coefficients at a glance.
...
Calling `classic()` on a t test with
[`prior_flat()`](https://norimune.github.io/BayesRTMB/reference/prior_flat.md)
displays results in a form close to an ordinary t test.
...
is the standardized effect size. Use `classic()` when you want to
inspect a BayesRTMB model as a frequentist analysis.
...
`fixed = list(delta = 0)` specifies a comparison against the null model
where the effect size is fixed at 0. In this example, `Model 1`, the
model that estimates the effect size, is supported.

For real analyses, use more MCMC samples than shown here and check that
the Bayes factor is stable.

## Next Steps

This page covered only the entry points to BayesRTMB. Continue with the
page that matches your purpose.

1.  **[Wrapper
    Functions](https://norimune.github.io/BayesRTMB/articles/wrapper_functions.md)**  
    Learn how to run standard analyses such as regression, GLM, mixed
