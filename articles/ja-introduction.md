...
の `generate` ブロックで、観測ごとの対数尤度を `log_lik`
という変数に格納しておくと、推定後に `$WAIC()` メソッドを使って
WAIC（広く使える情報量基準）を計算できます。

たとえば、手書きモデルの `generate` ブロックに以下のように `log_lik`
を定義します。
...
※
ラッパー関数（[`rtmb_lm()`](https://norimune.github.io/BayesRTMB/reference/rtmb_lm.md)
など）を使う場合は、`WAIC = TRUE` を引数に指定することで、自動的に内部で
`log_lik` が生成されます。
...
`WAIC()` メソッドは、MCMC (`MCMC_Fit`) や変分推論 (`VB_Fit`)、および
`se_method = "sampling"` を指定した MAP 推定 (`MAP_Fit`)
の結果に対して利用できます。
...
6.  [**分析リファレンス**](https://norimune.github.io/BayesRTMB/articles/ja-analysis_reference.md)  
