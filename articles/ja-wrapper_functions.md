...
回帰、GLM、GLMM
のラッパーで交互作用を含むモデルを推定した場合は、[`conditional_effects()`](https://norimune.github.io/BayesRTMB/reference/conditional_effects.md)
や
[`simple_effects()`](https://norimune.github.io/BayesRTMB/reference/simple_effects.md)
を使って、予測値や単純効果を確認できます。
...
MCMC fit では事後サンプルに基づいて区間を計算します。MAP fit や classic
fit
でも、標準誤差やシミュレーションに基づく不確実性が利用できる場合には、同じ関数で条件付き効果や単純効果を確認できます。
